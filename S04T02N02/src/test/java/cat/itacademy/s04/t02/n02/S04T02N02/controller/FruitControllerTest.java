...
class FruitControllerTest {
    ...
    void createFruit_shouldReturnCreatedFruit() throws Exception {
        ...
        Fruit savedFruit = new Fruit(1L , "Cherry", 1L);
    ...
    void updateFruit_shouldReturnUpdatedFruit() throws Exception {
        ...
        Mockito.when(fruitService.getOneFruit(1L))
                .thenReturn(new Fruit(1L, "Apple", 5L));

        Mockito.when(fruitService.updateFruit(Mockito.any(Fruit.class)))
                .thenReturn(new Fruit(1L, "Pear", 9L));
    ...
    void deleteFruit_shouldReturnNoContent() throws Exception {
        ...
        Mockito.when(fruitService.getOneFruit(1L)).thenReturn(existing);
        Mockito.doNothing().when(fruitService).deleteFruit(1L);
    ...
    void getOneFruit_shouldReturnSingleFruit() throws Exception {
        Fruit fruit = new Fruit(1L , "Apple", 8L);

        Mockito.when(fruitService.getOneFruit(1L)).thenReturn(fruit);
    ...
    void getAllFruits_shouldReturnList() throws Exception {
        List<Fruit> fruits = List.of(
                new Fruit(1L , "Strawberry", 10L),
                new Fruit(2L , "Kiwi", 5L)
        );
